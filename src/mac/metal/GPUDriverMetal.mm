...
GPUDriverMetal::GPUDriverMetal(GPUContextMetal* context)
    : context_(context)
{
}
    ...
    const IndexBuffer& indices)
{
    ...
    UpdateGeometryResource(geometry, vertices, indices);
}
...
void GPUDriverMetal::ApplyScissor(const GPUState& state)
{
    ...
    if (state.enable_scissor) {
        ...
        if (r.x < render_encoder_render_buffer_width_ && r.y < render_encoder_render_buffer_height_) {
...
void GPUDriverMetal::UpdateGeometryResource(GeometryEntry& geometry, const VertexBuffer& vertices,
    const IndexBuffer& indices)
{
    if (render_encoder_) {
        [render_encoder_ endEncoding];
    ...
    id<MTLBuffer> uploadBuffer;
    uploadBuffer = [context_->device() newBufferWithBytes:vertices.data
                                                   length:vertices.size
                                                  options:MTLResourceStorageModeShared];
    blit_encoder_ = [context_->command_buffer() blitCommandEncoder];
    [blit_encoder_ copyFromBuffer:uploadBuffer
                     sourceOffset:0
                         toBuffer:geometry.vertex_buffer
                destinationOffset:0
                             size:vertices.size];
    [blit_encoder_ endEncoding];
    blit_encoder_ = nullptr;

    uploadBuffer = [context_->device() newBufferWithBytes:indices.data
                                                   length:indices.size
                                                  options:MTLResourceStorageModeShared];
    blit_encoder_ = [context_->command_buffer() blitCommandEncoder];
    [blit_encoder_ copyFromBuffer:uploadBuffer
                     sourceOffset:0
                         toBuffer:geometry.index_buffer
                destinationOffset:0
                             size:indices.size];
    [blit_encoder_ endEncoding];
    blit_encoder_ = nullptr;
