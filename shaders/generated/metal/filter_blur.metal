...
fragment main0_out main0(main0_in in [[stage_in]], constant type_Uniforms& Uniforms [[buffer(0)]], texture2d<float> Texture0 [[texture(0)]], sampler Sampler0 [[sampler(0)]])
{
    main0_out out = {};
    float4 _78;
    _78 = (Texture0.sample(Sampler0, fast::clamp(in.in_var_TEXCOORD0, float2(0.0), float2(1.0))) * in.in_var_COLOR0) * _62[0];
    for (int _81 = 1; _81 < 11; )
    {
        float _85 = float(_81);
        float2 _91 = float2(Uniforms.Scalar4[0].x, Uniforms.Scalar4[0].y);
        _78 = (_78 + ((Texture0.sample(Sampler0, fast::clamp(in.in_var_TEXCOORD0 + (_91 * (_85 * 0.20000000298023223876953125)), float2(0.0), float2(1.0))) * in.in_var_COLOR0) * _62[_81])) + ((Texture0.sample(Sampler0, fast::clamp(in.in_var_TEXCOORD0 + (_91 * (_85 * (-0.20000000298023223876953125))), float2(0.0), float2(1.0))) * in.in_var_COLOR0) * _62[_81]);
        _81++;
        continue;
    }
